...
void main(void)
{
	...
	if(vPosOrig.z > (uTailleBoite - 0.1) && vPosOrig.z < (uTailleBoite + 0.1)){
		col = texture2D(uSampler0, vec2(texCoords.s, texCoords.t));
	}
	else if(vPosOrig.z > (-uTailleBoite - 0.1) && vPosOrig.z < (-uTailleBoite + 0.1)){
		col = texture2D(uSampler1, vec2(texCoords.s, texCoords.t));
	}
	else if(vPosOrig.y > (-uTailleBoite - 0.1) && vPosOrig.y < (-uTailleBoite + 0.1)){
		col = texture2D(uSampler2, vec2(texCoords.s, texCoords.t));
	}
	else if(vPosOrig.y > (uTailleBoite - 0.1) && vPosOrig.y < (uTailleBoite + 0.1)){
		col = texture2D(uSampler3, vec2(texCoords.s, texCoords.t));
	}
	else if(vPosOrig.x > (uTailleBoite - 0.1) && vPosOrig.x < (uTailleBoite + 0.1)){
